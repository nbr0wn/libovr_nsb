...
void main( void ) {
    ...
    vec3 ray = rotate(ray,vec2(-0.5,1.-time*0.05));
    float radius = length(ray);
    float phi = asin(ray.y / radius);
    float theta = atan(ray.x, ray.z);
    vec2 p = vec2( theta, phi*2.0 ) / pi;
	vec3 pos = normalize(ray);

	
	//vec2 m = (mouse/resolution-0.5)*pi*vec2(2.,1.);
